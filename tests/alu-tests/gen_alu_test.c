...
/* gcc gen_test.c && ./a.out > test.c && movcc test.c
 * movfusator/lib/softfloat64.o && ./a.out | grep FAIL */
...
static const signed int vsi[] = {0x80000000, 0x80000001, -2,        -1, 0, 1,
                                 2,          0x7ffffffe, 0x7fffffff};
static const signed short vss[] = {0x8000, 0x8001, -2,     -1,    0,
                                   1,      2,      0x7ffe, 0x7fff};
static const signed char vsc[] = {0x80, 0x81, -2, -1, 0, 1, 2, 0x7e, 0x7f};

static const unsigned int vui[] = {0x80000000, 0x80000001, -2,        -1, 0, 1,
                                   2,          0x7ffffffe, 0x7fffffff};
static const unsigned short vus[] = {0x8000, 0x8001, -2,     -1,    0,
                                     1,      2,      0x7ffe, 0x7fff};
static const unsigned char vuc[] = {0x80, 0x81, -2, -1, 0, 1, 2, 0x7e, 0x7f};

#define TEST_STRING                                                            \
  "  printf(\"line %%d: %%s: %%d  %%s  %%d  ==  %%d =>  %%s (%%d)\\n\","       \
  "__LINE__,\"%s\",0x%x,\"%s\",0x%x,0x%x,(%s)(x%sy)==0x%x?\"PASS\":\"FAIL\",(" \
  "%s)(x%sy));\n"
...
#define TEST_PARMS(type, set, op)                                              \
  #type, set[i], #op, set[j], (type)(set[i] op set[j]), #type, #op,            \
      (type)(set[i] op set[j]), #type, #op
